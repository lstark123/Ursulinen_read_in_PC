...
def load_flights_of_the_day():
    ...
    try:
        ...
        arrivals_sp = os.path.join(flights_save_path, (datetime.datetime.now().strftime("%Y-%m-%d") + "_arrivals" + ".csv"))
        arrivals.to_csv(arrivals_sp, index=False)  # Set index=False to exclude the index column
        print(f'Arrivals saved to {arrivals_sp}')
        departures_sp = os.path.join(flights_save_path, (datetime.datetime.now().strftime("%Y-%m-%d")+"_departures" + ".csv"))
        ...
        logging.save_logging("Finished loading flights, see you tomorrow at 23:00. \n ------------------------")
    except Exception as error:
        logging.give_error()
...
def main():
    ...
    logging.save_logging(f"Getting the flights every day at {specific_time}")
